...
describe("payout phase constraints", () => {
  it("one payout callback per round", () => {
    ...
    state.payoutTimer = 2.0; // Assume 2.0s is past the coin completion
    
    const onResult = vi.fn();
    // Simulate what tickCaptureState would do when time completes (if we put it in tickCaptureState, but wait, resultFired is set manually in createDockRuntime. Let's just check the boolean flip pattern)
    // Actually, createDockRuntime handles this. Let's just mock the condition.
